...
class productClass:
    def __init__(self, root):
        ...
        #=====title======
        title=Label(product_Frame, text = "Manage Product Details", font =("goudy old style", 18), bg ="white", fg ="white").pack(side=TOP, fill=X)
        lbl_category=Label(product_Frame, text = "Category", font =("goudy old style", 18), bg ="white").place(x=30, y=60)
        lbl_product=Label(product_Frame, text = "Supplier", font =("goudy old style", 18), bg ="white").place(x=30, y=110)
        ...
        ##options
        cmb_search=ttk.Combobox(SearchFrame,textvariable=self.var_searchby,values=("Select","Category","product","Name"),state="readonly",justify=CENTER,font=("goudy old style",15,))
    ...
    ###table
        ...
        p_frame.place(x=480,y=100,width=650,height=390)
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Product is already available, try Different",parent=self.root)
                else:
